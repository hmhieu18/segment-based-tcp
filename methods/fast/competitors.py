...
import methods.fast.lsh as lsh
...
def ga_s(input_file):
    def storeSpanningFile(input_file, spanfile):
        ...
        with open(spanfile + ".tmp", "w") as fout:
            subprocess.call(["perl", "methods/fast/subsume.pl", matrixFile], stdout=fout)
    ...
    spantimefile = "{}_spantime.txt".format(os.path.basename(input_file).split(".")[0])
    spantimefile = os.path.join(os.path.dirname(input_file), spantimefile)
    # if not os.path.exists(spanfile):
        # time.time() does not consider subprocess call time
    span_t = time.time()
...
def str_(input_file):
    ...
    if not os.path.exists(sigfile):
        ...
        ledru_time = time.time() - ledru_t
        input(os.path.basename(input_file).split(".")[0])
        with open("{}_sigtime.txt".format(os.path.basename(input_file).split(".")[0]), "w") as fout:
            fout.write(repr(ledru_time))
    else:
        input(os.path.basename(input_file).split(".")[0])
        with open("{}_sigtime.txt".format(os.path.basename(input_file).split(".")[0]), "r") as fin:
